    ...
    try
    {
      ...
      for (i=0; i<nBlobs; i++)
      {
	    ...
	    if (c==0)
	    {
            ...
            track->centroid_list.push_back(blob->centroid);
            track->active = 0;
            track->inactive = 0;
      ...
      for (j=0; j<nTracks; j++)
      {
        ...
        if (c)
        {
	        ...
	        track->label = blob->label;
	        track->centroid = blob->centroid;          
            track->centroid_list.push_back(blob->centroid);//history center information
	        track->minx = blob->minx;
	        track->miny = blob->miny;
	        track->maxx = blob->maxx;
	        track->maxy = blob->maxy;
     ...
     for (CvTracks::iterator jt=tracks.begin(); jt!=tracks.end();)
        ...
        else
        {
           ...
           // if(jt->second->centroid_list.size()>21)
             //   jt->second->centroid_list.pop_front();
  ...
  void cvRenderTracks(CvTracks const tracks, IplImage *imgSource, IplImage *imgDest, unsigned short mode, CvFont *font)
  {
    ...
    if ((mode&CV_TRACK_RENDER_ID)&&(!font))
    {
      if (!defaultFont)
      {
	    ...
	    cvInitFont(font, CV_FONT_HERSHEY_DUPLEX, 0.5, 0.5, 0, 2);
	    // Other fonts:
      ...
      for (CvTracks::const_iterator it=tracks.begin(); it!=tracks.end(); ++it)
      {
	    ...
	    if (mode&CV_TRACK_RENDER_BOUNDING_BOX)
	      if (it->second->inactive)
          {
            ...
            /*int num_point = 0;
            for(std::list<CvPoint2D64f>::iterator _it = it->second->centroid_list.begin();_it!=it->second->centroid_list.end(),num_point<it->second->centroid_list.size()-1;num_point++)
            {               
                CvPoint tracking_center1,tracking_center2;
                tracking_center1.x = (int)(*_it).x; 
                tracking_center1.y = (int)(*_it).y;
                ++_it;//next point
                tracking_center2.x = (int)(*_it).x; 
                tracking_center2.y = (int)(*_it).y;
                cvLine(imgDest,tracking_center1,tracking_center2,CV_RGB(0., 0., 255.),1);                
            }*/
          }
	      else
          {
	        cvRectangle(imgDest, cvPoint(it->second->minx, it->second->miny), cvPoint(it->second->maxx-1, it->second->maxy-1), CV_RGB(0., 0., 255.),3);
            int _size = 0;
            /*for(std::list<CvPoint2D64f>::iterator _it = it->second->centroid_list.begin();_it!=it->second->centroid_list.end(),_size<it->second->centroid_list.size()-1;_size++)
            {               
                CvPoint tracking_center1,tracking_center2;
                tracking_center1.x = (int)(*_it).x; 
                tracking_center1.y = (int)(*_it).y;
